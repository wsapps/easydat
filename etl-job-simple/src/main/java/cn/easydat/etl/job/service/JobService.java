		...
		if (null != taskInfo) {
			...
			try {
				if (status == -1) {
					...
					sqlUtils.sql("update etl_job_task_process set run_status=2,run_time_start=?,run_time_end=?,runtime=?,reader_row_num=?,writer_row_num=? where id=?", new Date(finishInfo.getStartTime()),
	...
	private JobParameter taskInfoToJobParameter(Map<String, Object> taskInfo) {
		...
		if (null != taskInfo) {
			Integer taskId = (Integer) taskInfo.get("task_id");
				/**
				 SELECT
					...
					a.channel 'setting.channel',
					a.max_num_of_channel 'setting.maxNumOfChannel',
					a.batch_size 'writer.batchSize'
				FROM
					etl_task a
			...
			String queryTaskSql = "SELECT\r\n"
				...
				+ "	w.username 'writer.jdbc.username',\r\n"
				+ "	w.pwd 'writer.jdbc.password',\r\n"
				+ " a.channel 'setting.channel',a.max_num_of_channel 'setting.maxNumOfChannel',a.batch_size 'writer.batchSize'"
				+ "FROM\r\n"
				+ "	etl_task a\r\n"
				+ "	LEFT JOIN etl_datasource r ON a.r_ds_id = r.id\r\n"
				+ "	LEFT JOIN etl_datasource w ON a.w_ds_id = w.id \r\n"
